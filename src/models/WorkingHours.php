...
class WorkingHours extends Model
{
    ...
    public static function getAusenteUser() 
    {
        $hoje = new DateTime();
        $resultado = Database::getResultFromQuery("
        SELECT name FROM users
        WHERE end_date is NULL
        AND id NOT IN (
        SELECT user_id FROM working_hours
        WHERE work_date='{$hoje->format('Y-m-d')}'
    ...
    ");
        ...
        if ($resultado->num_rows > 0) {
            while ($row = $resultado->fetch_assoc()) {
                array_push($ausenteUsers, $row['name']);
            }
        }
    ...
    public static function getHorasTrabalhadasNoMes($anoMes){
        $startDate=(new DateTime("{$anoMes}-1"))->format('Y-m-d');
        $endDate=getUltimoDiaDoMes($anoMes)->format('Y-m-d');
        $resultado=static::getResultSetFromSelect([
            'raw'=>"work_date between '{$startDate}' and '{$endDate}'"
        ],"sum(worked_time) as sum");

        return $resultado->fetch_assoc()["sum"];
    }
